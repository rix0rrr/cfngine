...
export class Evaluator {
  ...
  public evaluate(x: TemplateExpression): any {
    ...
    switch (x.type) {
      ...
      case 'array': return this.evaluateArray(x.array);
      case 'object': return this.evaluateObject(x.fields);
      case 'intrinsic':
        switch (x.fn) {
          case 'base64': return this.fnBase64(assertString(ev(x.expression)));
          case 'cidr': return this.fnCidr();
          case 'findInMap': return this.fnFindInMap(x.mappingName, assertString(ev(x.key1)), assertString(ev(x.key2)));
          case 'getAtt': return this.fnGetAtt(x.logicalId, assertString(ev(x.attribute)));
          case 'getAzs': return this.fnGetAzs(assertString(ev(x.region)));
          case 'if': return this.fnIf(x.conditionName, x.then, x.else);
          case 'importValue': return this.fnImportValue(assertString(ev(x.export)));
          case 'join': return this.fnJoin(x.separator, assertList(ev(x.array)).map(assertString));
          case 'ref': return this.ref(x.logicalId);
          case 'select': return this.fnSelect(assertNumber(ev(x.index)), assertList(ev(x.array)));
          case 'split': return this.fnSplit(x.separator, assertString(ev(x.value)));
          case 'sub': return this.fnSub(x.fragments, this.evaluateObject(x.additionalContext));
          case 'transform': return this.fnTransform(x.transformName, this.evaluateObject(x.parameters));
          case 'and': return this.fnAnd(x.operands.map(ev).map(assertBoolean));
          case 'or': return this.fnOr(x.operands.map(ev).map(assertBoolean));
          case 'not': return this.fnNot(assertBoolean(ev(x.operand)));
          case 'equals': return this.fnEquals(ev(x.value1), ev(x.value2));
        }
    }
  }

  public evaluateObject(xs: Record<string, TemplateExpression>): Record<string, unknown> {
    return Object.fromEntries(Object.entries(xs)
  ...
  protected fnFindInMap(mappingName: string, key1: string, key2: string) {
    ...
    if (!inner) { throw new Error(`Mapping ${mappingName} has no key '${key1}' (available: ${Object.keys(map)})`); }
    const ret = inner.get(key2);
    if (ret === undefined) { throw new Error(`Mapping ${mappingName}[${key1}] has no key '${key2}' (available: ${Object.keys(inner)})`); }
    return ret;
  }
  ...
  protected fnGetAzs(region: string) {
    ...
    // FIXME: Empty string == current region
    return this.context.azs(region) ?? [];
  }

  protected fnIf(conditionName: string, ifYes: TemplateExpression, ifNo: TemplateExpression) {
  ...
  protected fnJoin(separator: string, array: string[]) {
    return array.join(separator);
  }

  protected ref(logicalId: string) {
    const c = this.context.referenceable(logicalId);
    if (!c) { throw new Error(`Ref: unknown identifier: ${logicalId}`); }
  ...
  protected fnSub(fragments: SubFragment[], additionalContext: Record<string, unknown>) {
  ...
  protected fnTransform(_transformName: string, _parameters: Record<string, unknown>) {
